...
@app.route('/period_reviewees/<int:period_id>')
@login_required
@admin_required
...
@app.route('/period_reviewers/<int:period_id>')
@login_required
@admin_required
...
@app.route('/update_period_reviewees/<int:period_id>', methods=['POST'])
@login_required
@admin_required
def update_period_reviewees(period_id):
    """Update reviewees for an assessment period"""
    period = AssessmentPeriod.query.get_or_404(period_id)
...
@app.route('/update_period_reviewers/<int:period_id>', methods=['POST'])
@login_required
@admin_required
def update_period_reviewers(period_id):
    """Update reviewers for an assessment period"""
    period = AssessmentPeriod.query.get_or_404(period_id)
...
def view_all_officer_reviews(officer_id):
    ...
    try:
        ...
        for assignment in assignments:
            ...
            if responses:  # Only include if there are responses
                # Group responses by category/form section
                grouped_responses = {}
                for response in responses:
                    question = AssessmentQuestion.query.get(response.question_id)
                    if question:
                        form = AssessmentForm.query.get(question.form_id)
                        category = form.title if form else 'General'
                        
                        if category not in grouped_responses:
                            grouped_responses[category] = []
                        
                        grouped_responses[category].append({
